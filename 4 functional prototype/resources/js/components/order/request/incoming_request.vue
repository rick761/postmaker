<template>
    <v-list-item >
        ...
        <v-list-item-avatar v-if="item.postmaker && item.postmaker.avatar">
            <v-img :src="'./storage/'+item.postmaker.id+'/avatar/'+item.postmaker.avatar"></v-img>
        </v-list-item-avatar>
        ...
        <v-list-item-content v-if="item.status == 'open' ">
            ...
            <modal v-model="acceptModal" width=500 title="Weet u het zeker?">
                ...
                En wordt het bedrag van {{item.payment}} euro afgesproken bij de oplevering.
